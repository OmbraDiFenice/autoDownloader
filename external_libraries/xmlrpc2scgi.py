...
class SCGIRequest(object):
    ...
    def send(self, data):
        ...
        complete_message = self.add_required_scgi_headers(data)
        encoded_message = complete_message.encode("utf-8")
        scgi_resp = self.__send(encoded_message)
        decoded_scgi_resp = scgi_resp.decode("utf-8")
        resp, self.resp_headers = self.get_scgi_resp(decoded_scgi_resp)
        return resp
...
if __name__ == "__main__":
    try:
        ...
        print("xmlrpc.client.Fault({0}): {1}".format(e.faultCode, e.faultString))
